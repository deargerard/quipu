...
if(accesoadm($cone,$_SESSION['identi'],16)){
	...
	if(isset($_POST['acc']) && !empty($_POST['acc'])){
		...
		if($acc=="agrasig"){
			if(isset($_POST['mon']) && !empty($_POST['mon']) && isset($_POST['tip']) && !empty($_POST['tip']) && isset($_POST['med']) && !empty($_POST['med']) && isset($_POST['num']) && !empty($_POST['num']) && isset($_POST['met']) && !empty($_POST['met']) && isset($_POST['feca']) && !empty($_POST['feca']) && isset($_POST['mes']) && !empty($_POST['mes'])){
				...
				if(mysqli_query($cone,"INSERT INTO teasignacion (mes, fecha, monto, tipo, idtemeta, empleado, medio, nummedio) VALUES ('$mes', '$feca', $mon, $tip, $met, $idu, $med, $num)")){
					$r['e']=true;
