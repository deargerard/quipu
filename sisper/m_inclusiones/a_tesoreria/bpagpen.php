...
if(accesocon($cone,$_SESSION['identi'],16)){
    ...
    $c1=mysqli_query($cone,"select tc.tipo, g.fechacom, g.numerocom, g.glosacom, g.totalcom from tegasto g INNER JOIN tetipocom tc ON g.idtetipocom=tc.idtetipocom INNER JOIN terendicion r ON g.idterendicion=r.idterendicion INNER JOIN temeta m ON r.idtemeta=m.idtemeta INNER JOIN tefondo f ON m.idtefondo=f.idtefondo WHERE f.idtefondo=$fon AND g.idteentrega is null AND g.idComServicios is null ORDER BY g.fechacom DESC;");
