      ...
      if($acc=="agrdr"){
        if(isset($_POST['doc']) && !empty($_POST['doc'])){
          ...
          if($doc==2){
            ...
            if(isset($_POST['idcs']) && !empty($_POST['idcs']) && isset($_POST['con']) && !empty($_POST['con']) && isset($_POST['mon']) && !empty($_POST['mon']) && isset($_POST['fecg']) && !empty($_POST['fecg'])){
              ...
              $cen=mysqli_query($cone,"SELECT idEmpleado FROM comservicios  WHERE idComServicios=$idcs;");
                ...
                if(mysqli_query($cone,"INSERT INTO tegasto (fechacom, glosacom, totalcom, idtetipocom, idteespecifica, idDependencia, idComServicios, idteconceptov ) VALUES ('$fecg', '$glo', '$mon', $doc, $ides, $idde, $idcs, $con);")){
                  $r['i']=mysqli_insert_id($cone);
                  $r['e']=true;
                  $r['m']=mensajesu("Listo, declaración jurada registrada");          
                }else{
            ...
            if(isset($_POST['idcs']) && !empty($_POST['idcs']) && isset($_POST['ser']) && !empty($_POST['ser']) && isset($_POST['num']) && !empty($_POST['num']) && isset($_POST['con']) && !empty($_POST['con']) && isset($_POST['mon']) && !empty($_POST['mon']) && isset($_POST['pro']) && !empty($_POST['pro']) && isset($_POST['fecg']) && !empty($_POST['fecg'])){

              $idcs=iseguro($cone,$_POST['idcs']);
              $con=iseguro($cone,$_POST['con']);
              $num=ltrim(iseguro($cone,$_POST['num']),"0");
              $ser=ltrim(iseguro($cone,$_POST['ser']),"0");         
              $mon=iseguro($cone,$_POST['mon']);
              $glo=iseguro($cone,$_POST['glo']);
          ...
          $r['m']=mensajewa("Elija el tipo de documento");
        }
      }if($acc=="edidr"){
        if(isset($_POST['doc']) && !empty($_POST['doc'])){
          $doc=iseguro($cone,$_POST['doc']);
          if($doc==2){
            ...
            if(isset($_POST['idcs']) && !empty($_POST['idcs']) && isset($_POST['idg']) && !empty($_POST['idg']) && isset($_POST['con']) && !empty($_POST['con']) && isset($_POST['mon']) && !empty($_POST['mon']) && isset($_POST['fecg']) && !empty($_POST['fecg'])){
              ...
              $cen=mysqli_query($cone,"SELECT idEmpleado FROM comservicios  WHERE idComServicios=$idcs;");
                ...
                mysqli_free_result($cen);         
                if(mysqli_query($cone,"UPDATE tegasto SET fechacom='$fecg', numerocom=null, glosacom='$glo', totalcom='$mon', idtetipocom=$doc, idteproveedor=null, idteespecifica=$ides, idDependencia=$idde, idteconceptov=$con WHERE idtegasto=$idg;")){
                  $r['e']=true;
                  $r['m']=mensajesu("Listo, declaración jurada actualizada");
                }else{
                  $r['m']=mensajewa("Error, intentelo nuevamente o contacte con el administrador");
                }
            }else{
              $r['m']=mensajewa("Todos los campos son obligatorios");
            }
            ...
            if(isset($_POST['idcs']) && !empty($_POST['idcs']) && isset($_POST['idg']) && !empty($_POST['idg']) && isset($_POST['ser']) && !empty($_POST['ser']) && isset($_POST['num']) && !empty($_POST['num']) && isset($_POST['con']) && !empty($_POST['con']) && isset($_POST['mon']) && !empty($_POST['mon']) && isset($_POST['pro']) && !empty($_POST['pro']) && isset($_POST['fecg']) && !empty($_POST['fecg'])){
