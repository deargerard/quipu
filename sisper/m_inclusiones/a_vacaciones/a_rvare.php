...
if(accesocon($cone,$_SESSION['identi'],3)){
	...
	$estvac=$_POST["estvac"];
	$convac=$_POST["convac"];
	if(isset($reglab) && isset($pervac) && isset($estvac) && isset($convac)){
		...
		for ($l=0; $l < count($convac); $l++) {
			...
			$c="SELECT e.NumeroDoc, e.idEmpleado, c.Denominacion as cargo, d.Denominacion, cl.Tipo, ec.FechaVac, pva.idPeriodoVacacional, pva.PeriodoVacacional, pv.FechaIni, pv.FechaFin, pv.Estado FROM provacaciones pv INNER JOIN empleadocargo ec ON pv.idEmpleadoCargo=ec.idEmpleadoCargo INNER JOIN empleado e ON ec.idEmpleado=e.idEmpleado INNER JOIN condicionlab cl ON ec.idCondicionLab=cl.idCondicionLab INNER JOIN cargo c ON ec.idCargo=c.idCargo INNER JOIN cardependencia cd ON ec.idEmpleadoCargo=cd.idEmpleadoCargo INNER JOIN dependencia d ON cd.idDependencia=d.idDependencia INNER JOIN periodovacacional pva ON pv.idPeriodoVacacional=pva.idPeriodoVacacional WHERE cd.Oficial=1 AND $wrl AND $wpv AND $wev AND $wcv";
			//echo $c;
