...
if(accesoadm($cone,$_SESSION['identi'],4)){
  if(isset($_POST['idli']) && !empty($_POST['idli'])){
    ...
    $cli=mysqli_query($cone,"SELECT * FROM licencia WHERE idlicencia=$idli");
    if($rli=mysqli_fetch_assoc($cli)){
        ?>

        <div class="col-sm-12 text-center">
          <input type="hidden" name="idli" value="<?php echo $idli?>"> <!--envía id de personal-->
        </div>
        ...
        <div class="form-group valida">
          ...
          <div class="col-sm-4 has-feedback">
            ...
            <input type="text" id="inienc" name="inienc" class="form-control" value="<?php echo date('d/m/Y H:m', strtotime($rli['FechaIni']))?>" placeholder="dd/mm/aaaa H:i" autocomplete="off">
          </div>
          <label for="finenc" class="col-sm-2 control-label">Termina:</label>
          <div class="col-sm-4 has-feedback">
            <span class="fa fa-calendar form-control-feedback"></span>
            <input type="text" id="finenc" name="finenc" class="form-control" value="<?php echo date('d/m/Y H:m', strtotime($rli['FechaFin']))?>" placeholder="dd/mm/aaaa H:i" autocomplete="off">
          </div>
        </div>
    ...
    echo mensajewa("Error: No se selecciono ninguna Licencia.");
  }
}else{
