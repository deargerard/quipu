...
if(accesoadm($cone,$_SESSION['identi'],6)){
	if(isset($_POST["NomForm"]) && $_POST["NomForm"]=="f_edidependencia"){
		$jres=array();
		if(isset($_POST['iddep']) && !empty($_POST['iddep']) && isset($_POST['pad']) && !empty($_POST['pad']) && isset($_POST['den']) && !empty($_POST['den']) && isset($_POST['sig']) && !empty($_POST['sig']) && isset($_POST['loc']) && !empty($_POST['loc']) && isset($_POST['disfis']) && !empty($_POST['disfis'])){
			...
			$sql="UPDATE dependencia SET idDistritoFiscal=$disfis, idLocal=$loc, Denominacion='$den', idDependenciaPadre=$pad, Siglas='$sig' WHERE idDependencia=$iddep";
