...
if(accesoadm($cone,$_SESSION['identi'],1) || accesoadm($cone,$_SESSION['identi'],9)){
	if(isset($_POST["NomForm"]) && $_POST["NomForm"]=="f_acttelefono"){
		if(isset($_POST['idte']) && !empty($_POST['idte'])){
			$idte=iseguro($cone,$_POST['idte']);
			$sql="UPDATE telefonoemp SET Estado=1 WHERE idTelefonoEmp=$idte";
			if(mysqli_query($cone,$sql)){
				echo "<h4 class='text-olive'>Listo: El teléfono fue activado.</h4>";
			}else{
