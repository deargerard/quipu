...
if(accesoadm($cone,$_SESSION['identi'],6)){
	if(isset($_POST["NomForm"]) && $_POST["NomForm"]=="f_actcoordinacion"){
		if(isset($_POST['idco']) && !empty($_POST['idco'])){
			$idco=iseguro($cone,$_POST['idco']);
			$sql="UPDATE coordinacion SET Estado=1 WHERE idCoordinacion=$idco";
			if(mysqli_query($cone,$sql)){
				echo mensajesu("Listo: Coordinación activada.");
			}else{
