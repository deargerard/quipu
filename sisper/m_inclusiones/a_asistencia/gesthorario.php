<?php
session_start();
include("../php/conexion_sp.php");
include("../php/funciones.php");
if(accesoadm($cone,$_SESSION['identi'],2)){
  $r=array();
  if(isset($_POST['idh']) && !empty($_POST['idh'])){
    $idh=iseguro($cone,$_POST['idh']);
    $ch=mysqli_query($cone,"SELECT Estado FROM horario WHERE idHorario=$idh;");
    if($rh=mysqli_fetch_assoc($ch)){
      $est=$rh['Estado']==1 ? 0 : 1;
      if(mysqli_query($cone,"UPDATE horario SET Estado=$est WHERE idHorario=$idh;")){
        $r['m']=mensajesu("Se cambio el estado del horario.");
        $r['e']=true;
      }else{
        $r['m']=mensajewa("No se pudo cambiar el estado. Intentelo nuevamente.");
        $r['e']=false;
      }
    }else{
      $r['m']=mensajewa("Los datos no son válidos.");
      $r['e']=false;
    }
  }else{
    $r['m']=mensajewa("No envió datos.");
    $r['e']=false;
  }
  header('Content-type: application/json; charset=utf-8');
  echo json_encode($r);
  exit();
}else{
  echo accrestringidoa();
}
  mysqli_close($cone);
?>